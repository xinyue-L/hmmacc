#' The HMM-based Sleep/Wake Identification Algorithm Assuming Zero-Inflated Gaussian (Recommended)
#'
...
HMM_ZIG <- function(Yob,paramInit,tranInit,piInit,ntime=NULL,maxIter=20,reltol=10^-2) {
  ...
  emit <- function(o,sig1,mu1,sd1,mu2,sd2,state,dlog=F) {
    return(switch(state,
                  dnorm0(o,sig1,mu1,sd1,log=dlog),
  ...
  while (step < maxIter & abs(loglik_delta/loglik_ori) > reltol) {
    ...
    b_hat1 <- ZIGaussian(Yob,weights=tem_g2[1,])
